...
summary_list <- map(seq.int(num_replicates), \(ix) make_summary(read_mcmc(str_interp("out/s2/timtam-scenario-2-2-sample-$[03d]{ix}.log")), ix))
...
ggsave(
  filename = "out/s2/plots/effective-sample-sizes-s-2-2.png",
  plot = ess_gg,
...
ggsave(
  filename = "out/s2/plots/birth-rate-estimates-s-2-2.png",
  plot = birth_rate_gg,
  height = 10.5, width = 14.8,
  units = "cm"
)
...
ggsave(
  filename = "out/s2/plots/sampling-rate-estimates-s-2-2.png",
...
ggsave(
  filename = "out/s2/plots/omega-rate-estimates-s-2-2.png",
  plot = omega_rate_gg,
  height = 10.5, width = 14.8,
  units = "cm"
)
...
final_prevalence_df <-
  "out/s2/final-simulation-state.csv" |>
  read.csv() |>
  select(X, sample) |>
...
ggsave(filename = "out/s2/plots/history-size-estimates-s-2-2.png",
