...
sim_log_lines <- tail(readLines("out/s1/remaster-scenario-1.log"), -1)
...
write.table(
  x = sim_dfs,
  file = "out/s1/trajectories-scenario-1.csv",
  sep = ",",
  row.names = FALSE
)

recon_trees <- "out/s1/remaster-scenario-1.tree" |>
  readLines() |>
...
for (ix in seq.int(num_samples)) {
  ...
  writeLines(
    text = omega_str,
    con = str_interp("out/s1/occurrence-times-scenario-1-sample-$[03d]{ix}.ssv")
  )

  write.phyDat(
    tmp$seqs,
    str_interp("out/s1/sequences-scenario-1-sample-$[03d]{ix}.fasta"),
    format = "fasta"
  )
  write.tree(
    tmp$recon_tree,
    str_interp("out/s1/reconstruction-scenario-1-sample-$[03d]{ix}.tree")
  )
}
